...
public class ItemRepositoryHSQLImpl implements ItemRepository {
	...
	public Item getItemById(final Integer id) {
		Item item = null;
		Map<String, Object> rowMap = this.jdbcTemplate.queryForMap("SELECT * FROM ITEM WHERE ID = ?", id);
		
		if (rowMap != null) {
			item = new Item((Integer)rowMap.get("ID"), (String)rowMap.get("DESCRIPTION"), Double.valueOf(rowMap.get("ITEM_VALUE").toString()));
		}
