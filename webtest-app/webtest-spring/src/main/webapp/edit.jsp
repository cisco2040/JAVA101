...
<%!public ShipTo makeUser(HttpServletRequest request) {
...
if ( request.getParameter("Update") != null ) {
	...
	if (validateService.isValid()) { // if succesful, redirect to list, otherwise repaint form with error messages
   		response.sendRedirect("/webtest-spring/jsp/shipTo/list.jsp");		
   	}
}
...
if (request.getParameter("home") != null) {
	response.sendRedirect("/webtest-spring/index.jsp");
}

pageContext.setAttribute("frmValUserId", shipTo.getUser().getUsername() != null ? shipTo.getUser().getUsername() : null);
pageContext.setAttribute("frmValCityId", shipTo.getCity().getCityId() != null ? shipTo.getCity().getCityId() : null);
...
<body>	
	...
	<div style="margin-left:5%;margin-top: 1%;">
		<form action="/webtest-spring/jsp/shipTo/edit.jsp" method="post">
			...
			<select name="frmUserId">
				...
				<c:forEach var="user" items="${ users }">
					<option value="${ city.username }" <c:if test="${ shipTo.user.userId == frmValUserId }">selected</c:if>>${ user.username }</option>
				</c:forEach>
			</select><br>			
			<input type="text" name="frmUsername" value="${ shipTo.user }"></input><br>
			...
			<select name="frmCityId">
				...
				<c:forEach var="city" items="${ cities }">
					<option value="${ city.cityId}" <c:if test="${ shipTo.city.cityId == frmValCityId }">selected</c:if>>${ city.description } + ${ city.state.description }</option>
				</c:forEach>
			</select><br>
